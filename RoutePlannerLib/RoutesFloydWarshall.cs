...
namespace Fhnw.Ecnf.RoutePlanner.RoutePlannerLib
{
	...
	public class RoutesFloydWarshall: Routes
	{
        ...
        {
            return FindShortestRouteBetween(fromCity.Name, toCity.Name, mode);
        }

        public override List<Link> FindShortestRouteBetween(string fromCity, string toCity,TransportModes mode)
        {
            if (ExecuteParallel)
                return FindShortestRouteBetween_Parallel(fromCity, toCity, mode);
            else
                return FindShortestRouteBetween_Sequential(fromCity, toCity, mode);
        }

        private List<Link> FindShortestRouteBetween_Parallel(string fromCity, string toCity, TransportModes mode)
        {
            List<City> cities = this.cities.FindCitiesBetween(fromCity, toCity);
		...
		public List<Link> FindShortestRouteBetween_Sequential(string fromCity, string toCity,TransportModes mode)
		{
			List<City> cities =  this.cities.FindCitiesBetween(fromCity,toCity);
			if (cities == null || cities.Count < 1)
			{
